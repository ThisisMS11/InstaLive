...
import { HoveredLink, Menu, MenuItem, ProductItem, AvatarItem, LogoItem } from "./ui/navbar-menu";
...
function Avatar() {
    const router = useRouter();
    const session = useSession();
    return <div className={`inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-slate-950 text-sm font-medium text-white backdrop-blur-3xl ${session?.data?.user ? "" : "px-3 py-1"}`}
        onClick={async () => {
            session?.data?.user ? router.push("/dashboard") : await signIn();
        }}
    >
    ...
    return (
        ...
        >
            ...
            <Menu setActive={setActive}>
                ...
                {/* @ts-ignore */}
                {session?.data?.user && <AvatarItem setActive={setActive} active={active} item={session?.data?.user.image}>
                    <div className="flex flex-col space-y-4 text-sm">
                        <HoveredLink href="/hobby">Hobby</HoveredLink>
                        <HoveredLink href="/individual">Individual</HoveredLink>
                        <HoveredLink href="/team">Team</HoveredLink>
