...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        logger.info(`Banning livechatUser with id : [${authorChannelId}] with type : ${type}`);
...
export async function PUT(req: NextRequest) {
    try {
        ...
        return makeResponse(200, true, `Unblocked User with id`, { authorChannelId });
    } catch (error) {
        logger.error(`Error unblocking user: ${error}`);
        return makeResponse(500, false, `Error while Unblocking User with id`, null);
    }
}