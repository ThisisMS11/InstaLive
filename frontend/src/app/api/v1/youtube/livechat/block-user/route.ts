...
export async function POST(req: NextRequest) {
    ...
    try {
        const { messageId, liveChatId, authorChannelId, type } =
            await req.json();
        ...
        logger.info(
            `Blocked user ${authorChannelId} in live chat ${liveChatId}`
        );
        ...
        if (redisConnected && banResponse?.data) {
            ...
            logger.info(
                `Storing ban info for messageId: ${messageId} in Redis`
            );
            ...
            logger.warn(
                `Redis service is unavailable or ban response was empty.`
            );
        }
        ...
        return makeResponse(
            500,
            false,
            `Error while Blocked User with id`,
            null
        );
    }
}
...
export async function PUT(req: NextRequest) {
    try {
        ...
        if (redisConnected) {
            try {
                logger.info(
                    `Fetching banId for ${messageId} from messageBanId hash`
                );
                ...
                if (!messageBanData || !messageBanData.banId) {
                    logger.error(
                        `MessageBanData or banId is not available in Redis.`
                    );
                ...
                logger.info(
                    `Unblocked user ${authorChannelId} in live chat ${banId}`
                );
                return makeResponse(200, true, `Unblocked User`, {
                    authorChannelId,
                });
            } catch (error) {
...
export async function GET(req: Request) {
    try {
        ...
        if (!redisConnected) {
            logger.error(
                'Redis is not connected, fetching blocked user data failed'
            );
            return makeResponse(503, false, 'Redis Service Unavailable', null);
        }
